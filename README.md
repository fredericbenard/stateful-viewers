...
![Stateful viewing model](docs/architecture.svg)
...
- **Viewer Profile Generation** -- Generate a viewer profile, independent reflection style, initial internal state, short descriptions for each, and a label -- all in a single flow. Variability is ensured by parametric hints that randomly pin 2-4 of 7 dimensions per schema and let the LLM resolve the rest.
...
**Generate** -- Click **"Generate profile"** in the sidebar. This creates:
...
**Load** -- Click **"Load profile"** to see all available profiles (public and saved). The list is not filtered by LLM provider, so you can load a profile generated by one provider and use a different provider for reflections. Each entry shows the label and profile ID; click one to load it. The profile ID is always visible so you can match it to files in `data/profiles/` and `data/reflections/`.
...
**Saved profiles**
When you generate a profile **while running the dev server** (`npm run dev`), it is written to `data/profiles/<uuid>.json`. The file includes: UUID, generation timestamp, locale, LLM used, label, profile text (long + short), reflection style text (long + short), and initial state text (long + short).

**Public profiles**
Pre-generated profiles live in `data/profiles/public/` (EN + FR pairs). These are generated offline using `scripts/generate-profiles.ts` and are always available in the profile list.
...
**Generate public profiles** -- To generate (or regenerate) the public profiles:

```bash
npx tsx scripts/generate-profiles.ts
```

Generates 4 EN profiles with parametric hints, translates each to FR, and writes all 8 to `data/profiles/public/`.
...
**Retrofit / re-generate profile labels** -- Overwrite labels using a selected LLM provider (public + saved profiles):
