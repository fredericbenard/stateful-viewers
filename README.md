...
- **Public & Saved viewer artifacts** -- Load pre-generated public artifacts (tracked in git) and/or your own saved ones. Lists are not filtered by LLM provider — you can mix a profile generated by one LLM with reflections from another (and a style/state generated elsewhere).
...
- **Generate** — creates the artifact plus a short user-facing summary (and a label for profiles).
- **Load** — selects from public + saved artifacts. Lists are not filtered by provider, so you can mix providers across profile/style/state and reflections.
...
**Saved profiles**
When you generate a profile **while running the dev server** (`npm run dev`), it is written to `data/profiles/<uuid>.json`. The file includes: UUID, generation timestamp, locale, LLM used, label, and profile text (long + short).

**Saved reflection styles**
When you generate a reflection style (dev server), it is written to `data/styles/<uuid>.json`. The file includes: UUID, generation timestamp, locale, LLM used, and reflection style text (long + short).

**Saved initial states**
When you generate an initial internal state (dev server), it is written to `data/states/<uuid>.json`. The file includes: UUID, generation timestamp, locale, LLM used, and initial state text (long + short).
...
- `scripts/retrofit-labels.ts` -- Re-generate (or backfill) profile labels using a selected provider
...
```bash
# OpenAI (default)
npx tsx scripts/generate-profiles.ts 4 --llm openai --model gpt-5.2

# Anthropic / Claude
npx tsx scripts/generate-profiles.ts 4 --llm anthropic --model claude-sonnet-4-5-20250929

# Gemini (defaults to maxOutputTokens=4096 unless you override)
npx tsx scripts/generate-profiles.ts 4 --llm gemini --model gemini-3-pro-preview

# Ollama (local)
npx tsx scripts/generate-profiles.ts 4 --llm ollama --model llama3.1:8b-instruct-q5_K_M
```
...
- **Saved data** (dev + HF server): `data/profiles/`, `data/styles/`, `data/states/` (generated artifacts) plus `data/profiles/public/` (pre-generated public bundles, checked in) and `data/reflections/` (reflection sessions). Generated artifacts and reflections are gitignored. Saving and loading are implemented by the Node/Express server (`/api/save-profile`, `/api/save-style`, `/api/save-state`, `/api/save-reflection-session`, `/api/list-*`, `/api/load-*`). In dev, Vite proxies to this server; on Hugging Face Spaces, the same server runs inside the Docker container.
