...
from eval_pipeline.providers.ollama_provider import OllamaVisionProvider
...
def create_provider(name: str, model: str | None = None) -> VisionProvider:
    """Create a provider instance by name (openai, anthropic, gemini, ollama)."""
    ...
    if name == "ollama":
        return OllamaVisionProvider(model=resolved_model)

    raise ValueError(
        f"Unknown provider: {name!r}. Choose from: openai, anthropic, gemini, ollama."
    )
