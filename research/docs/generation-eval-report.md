...
All evaluation uses LLM-as-judge with a 1–5 rubric per criterion. Each stage has its own criteria (see §3). We test two generator models (GPT-5.2, Claude Opus 4.6) and two judge models (same pair), giving a 2×2 comparison with identical input prompts (via `--reuse-prompts`).
...
|---|:---:|:---:|:---:|:---:|
| emotional_depth | 4.0 | 4.2 | **4.4** | **4.6** |
| image_responsiveness | 4.2 | 4.4 | **4.6** | **4.8** |
| non_genericness | 4.6 | 4.2 | **4.8** | 4.4 |
| ambiguity_preservation | 3.0 | 2.8 | **3.2** | 2.8 |
| experiential_quality | 4.8 | 4.4 | **5.0** | 4.4 |
| state_evolution_quality | **5.0** | 4.0 | 4.8 | 4.8 |
| state_schema_coverage | **5.0** | **5.0** | 4.8 | **5.0** |
| profile_adherence | 4.6 | **5.0** | 4.6 | **5.0** |
| style_adherence | **5.0** | **5.0** | **5.0** | **5.0** |
| **Overall** | **4.5** | **4.3** | **4.6** | **4.5** |
...
**Claude generates richer reflections.** Claude→GPT is the highest overall (4.6), beating GPT→GPT (4.5). The advantages are clear: perfect experiential quality (5.0), higher emotional depth (4.4 vs 4.0), stronger image responsiveness (4.6 vs 4.2), and more distinctive language (4.8 vs 4.6). GPT itself recognizes Claude's superiority.

**Ambiguity preservation is the designed weak spot (2.8–3.2).** Both models correctly enact the low-ambiguity profile by resolving each image into a tidy takeaway ("modest insistence," "patching as quiet survival," "barrier with a crack," "layers of intention"). Both judges agree this is profile-correct behavior. Claude judge is slightly stricter here (2.8) than GPT judge (3.0–3.2), flagging more aggressively when the viewer "explicitly states satisfaction at having settled the meaning."
...
| Criterion | GPT→GPT | GPT→Claude | Claude→GPT | Claude→Claude |
|---|:---:|:---:|:---:|:---:|
| emotional_depth | 4.8 | **5.0** | 4.8 | **5.0** |
| image_responsiveness | 4.2 | 4.8 | 4.6 | **5.0** |
| non_genericness | **5.0** | **5.0** | **5.0** | **5.0** |
| ambiguity_preservation | 4.8 | **5.0** | **5.0** | **5.0** |
| experiential_quality | 4.8 | 4.6 | 4.8 | 4.6 |
| state_evolution_quality | 4.8 | 4.4 | **5.0** | **5.0** |
| state_schema_coverage | **5.0** | **5.0** | 4.8 | **5.0** |
| profile_adherence | **5.0** | **5.0** | **5.0** | **5.0** |
| style_adherence | 4.8 | **5.0** | **5.0** | **5.0** |
| **Overall** | **4.80** | **4.87** | **4.89** | **4.96** |
...
**Near-universal saturation.** 5 of 9 criteria reach a perfect 5.0 in ≥3 of 4 conditions; 7 of 9 score ≥4.8 in ≥3 conditions. `non_genericness` and `profile_adherence` are perfect 5.0 everywhere. This is dramatically higher than the low-ambiguity run (overall 4.3–4.6 vs. 4.8–4.96 here).
...
| Image responsiveness (stateful) | — | +0.4 avg in low-ambiguity; gap narrows in high-ambiguity (both ≥4.6) |
| Experiential quality (stateful) | — | +0.2 in low-ambiguity; tied in high-ambiguity (both 4.6–4.8) |
...
| `experiential_quality` | Generous to both (4.8 / 5.0) | Stricter on both (4.4 / 4.4) |
| `profile_adherence` | Conservative (4.6 for both) | Confident (5.0 for both) |
| `ambiguity_preservation` | Slightly more generous (3.0 / 3.2) | Slightly stricter (2.8 / 2.8) |
...
**Contrastive run update (§6b):** With the high-ambiguity profile, judge disagreement collapses. The largest gap drops to 0.4 (GPT gives `state_evolution_quality` 4.8 vs Claude's 4.4 for GPT output), suggesting the weary-to-awakening trajectory is genuinely clearer and both judges can recognize it. The models aren't just scoring higher — the quality difference that created judge disagreement has largely disappeared.
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **GPT → GPT** | 4.5 | 4.6 | 4.6 | 4.5 | 4.80 | **4.60** |
| **GPT → Claude** | 4.2 | 4.5 | 4.7 | 4.3 | 4.87 | **4.51** |
| **Claude → GPT** | 4.7 | 4.5 | 4.4 | 4.6 | 4.89 | **4.62** |
| **Claude → Claude** | 4.8 | 4.5 | 4.6 | 4.5 | 4.96 | **4.67** |

The overall means converge around 4.5–4.7. The high-ambiguity stateful run lifts all conditions by 0.3–0.5 relative to the low-ambiguity run, confirming that permissive profiles inflate scores across the board. The *ordering* of conditions is stable: Claude→Claude consistently scores highest, GPT→Claude lowest (Claude-as-judge is harder on GPT output). The text-only generation experiments remain the most discriminating test bed.
...
| `image_responsiveness` | Doesn't catch cross-image factual drift — Claude claimed "a lot of doors" when only 1 of 3 prior images showed doors, yet scored 5/5 | Add a sub-criterion or note in the rubric: "references to earlier images in the sequence must be factually accurate" |
...
| # | Category | Items | Ref |
|:---:|---|---|---|
| 1 | **Evaluation criteria** | Drop/merge saturated criteria, clarify rubrics for `pure_expressiveness`, `state_evolution_quality`, `image_responsiveness` | §7.4 |
...
- [ ] Add cross-image factual accuracy to `image_responsiveness` — the "a lot of doors" confabulation (§6.4) went unpenalized by both judges. No confabulation was observed in the high-ambiguity run (§6b.3), suggesting it may be profile-dependent.
